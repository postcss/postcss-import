...
const checkForCycles = require("./check-for-cycles")
...
async function loadImportContent(
  ...
  const atRule = stmt.node
  const stmtDuplicateCheckKey = createStmtDuplicateCheckKey(stmt)
  const { conditions, from } = stmt

  for (const condition of stmt.conditions) {
    assignLayerNames(condition.layer, atRule, state, options)
  }
  ...
  if (options.skipDuplicates) {
    ...
    if (!state.importEdges) {
      state.importEdges = []
    }

    const copy = [...state.importEdges, [from, filename]]

    if (checkForCycles(copy)) {
      return
    }

    state.importEdges.push([from, filename])
  }
...
function createStmtDuplicateCheckKey(stmt) {
  return JSON.stringify(
    stmt.conditions.map(x => [x.media.join(", "), x.layer.join(".")])
  )
}
